...
const EmployeeDetails = () => {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const nonAdmins = Array.isArray(res.data) ? res.data.filter(emp => emp.role !== "ADMIN") : [];

      // Flatten nested fields for table display
      // In EmployeeDetails.js - update the flattened mapping
const flattened = nonAdmins.map(emp => ({
  ...
  branch: emp.branch || "-",
  role: emp.role || "-",
  // Fix these lines to match your actual nested structure:
  designation: emp.professionalDetails?.areasOfExpertise || "-", // or whatever field represents designation
  experience: emp.professionalDetails?.totalExperience || "-",
  dob: emp.personalDetails?.dateOfBirth || "-",
  gender: emp.personalDetails?.gender || "-",
  department: emp.occupationalDetails?.employmentType || "-", // or shiftType
}));
  ...
  return (
    <div className="admin-dashboard container mt-4">
      <button className="btn btn-primary mb-3" onClick={() => navigate("/admin-dashboard")}>
        Back to Dashboard
      </button>
